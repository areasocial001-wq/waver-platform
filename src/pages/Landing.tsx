...
import { Sparkles, Video, Wand2, Mic, Layout, Zap, ArrowRight, CheckCircle2, Star, Quote, Play, Image as ImageIcon, Film, Music, ChevronRight, BookOpen, Volume2 } from "lucide-react";
...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-[hsl(225,25%,5%)] text-[hsl(210,40%,98%)] overflow-hidden">
        ...
        <motion.div style={{ opacity: heroOpacity }} className="relative container mx-auto px-4 text-center max-w-5xl">
          <motion.div initial="hidden" animate="visible" variants={staggerContainer}>
            ...
            <motion.h1 variants={fadeUp} custom={1} className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-extrabold tracking-tight mb-6 leading-[1.05] [-webkit-text-stroke:3px_black] [text-shadow:-3px_-3px_0_black,3px_-3px_0_black,-3px_3px_0_black,3px_3px_0_black,0_0_20px_black,0_0_40px_black,0_0_80px_hsl(225,25%,2%/0.95)]">
              ...
              <span className="block relative [-webkit-text-stroke:0px_transparent]" style={{ fontFamily: "'Montserrat', sans-serif" }}>
                {/* Stroke layer */}
                ...
                {/* Gradient fill layer */}
                <span className="absolute inset-0 bg-clip-text text-transparent [-webkit-text-stroke:0px_transparent] [filter:brightness(1.3)] animate-[metallic-shimmer_4s_ease-in-out_infinite] [background-size:200%_100%]" style={{ backgroundImage: 'linear-gradient(105deg, hsl(220,15%,75%) 0%, hsl(220,15%,75%) 35%, hsl(0,0%,100%) 50%, hsl(220,15%,75%) 65%, hsl(220,15%,75%) 100%)' }} aria-hidden="true">
                  Video & Image
                </span>
              </span>
      ...
      {/* Pricing section removed: commercial features disabled */}
      ...
      >
        <div className="container mx-auto px-4 text-center text-sm text-[hsl(215,20%,45%)]">
          <div className="grid grid-cols-3 items-center justify-items-center gap-6 mb-3 max-w-2xl mx-auto">
            <img src={logoImg} alt="AI Production Hub" className="w-24 h-24 rounded-full object-cover justify-self-end" />
            ...
            <img src={studiobookLogo} alt="StudioBook Edizioni" className="w-24 h-24 object-contain justify-self-start" />
          </div>
